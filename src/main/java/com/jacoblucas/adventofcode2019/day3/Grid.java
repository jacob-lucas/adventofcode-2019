...
public class Grid {
    static final Coordinates2D CENTRAL_PORT = ImmutableCoordinates2D.of(0, 0);
    static final GridEntry EMPTY = ImmutableGridEntry.builder().build();

    private Map<Coordinates2D, GridEntry> grid;
    ...
    public GridEntry get(final int x, final int y) {
        return grid.getOrElse(ImmutableCoordinates2D.of(x, y), EMPTY);
    }

    public Seq<Tuple2<Coordinates2D, GridEntry>> getWhere(final Predicate<GridEntry> predicate, final Comparator<Tuple2<Coordinates2D, GridEntry>> comparator) {
        return grid.filter(elem -> predicate.test(elem._2))
                .map(elem -> new Tuple2<>(elem._1, elem._2))
                .sorted(comparator);
    }

    public void trace(final String id, final Coordinates2D currentCoord, final List<String> paths) {
        if (!paths.isEmpty()) {
            final String next = paths.head();
            final char direction = next.charAt(0);
            int amount = Integer.parseInt(next.substring(1));
            final Coordinates2D destinationCoord;
            if (direction == 'U' || direction == 'D') {
                final int y = currentCoord.y();
                destinationCoord = ImmutableCoordinates2D.copyOf(currentCoord)
    ...
    void fillX(final Coordinates2D p, final Coordinates2D q, final String id) {
        if (p.y() == q.y()) {
            final int y = p.y();
    ...
    void fillY(final Coordinates2D p, final Coordinates2D q, final String id) {
        if (p.x() == q.x()) {
            final int x = p.x();
    ...
    void put(final int x, final int y, final String id) {
        final Coordinates2D coord = ImmutableCoordinates2D.of(x, y);
    ...
    public int steps(final List<String> paths, final Coordinates2D destination) {
        return steps(paths, CENTRAL_PORT, destination, 0);
    }

    private int steps(final List<String> paths, final Coordinates2D current, final Coordinates2D destination, final int stepCount) {
        if (!paths.isEmpty()) {
            final String next = paths.head();
            final char direction = next.charAt(0);
            int amount = Integer.parseInt(next.substring(1));
            final Coordinates2D updated;
            final int axisChange;
            if (direction == 'U' || direction == 'D') {
                final int y = current.y();
                axisChange = direction == 'U' ? y + amount : y - amount;
                updated = ImmutableCoordinates2D.copyOf(current).withY(axisChange);
                ...
                updated = ImmutableCoordinates2D.copyOf(current).withX(axisChange);
